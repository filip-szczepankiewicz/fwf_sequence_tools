...
% function [gwf, t, x] = optSDE.optimize(dur, tp, gmax, smax, hw, opt)
% By Filip Szczepankiewicz, Lund University
%
%   Designs a monopolar LTE diffusion-encoding gradient waveform (GWF) that maximizes
...
%          Default is created by optSDE.options(gmax, smax, dur).
...
%          useful waveforms please call optSDE.convert.ana2num(gwf, t, dt)
...
[gwf, t]  = optSDE.convert.par2gwf(x, dur, tp);
...
gwf = gwf*gmax;
    ...
    function cost = this_cost(v)
        ...
        % recompute flat time
        ftt  = dur - tmpA - tmpB;
        b    = optSDE.convert.par2bval(tmpA, tmpB, ftt, tp, gGlo);
        cost = -b;
    end
    ...
    function [C, Ceq] = this_nlcon(v)
        ...
        if opt.stimMode % If nerve stimulation is to be accounted for
            ...
            [gwfTmp, tTmp]   = optSDE.convert.par2gwf(v, dur, tp);
            [gwfI, rfi, dti] = optSDE.convert.ana2num(gwfTmp, tTmp, 0.01);
            gwf_tmp          = gwfI*gmax;
