...
% function bin_fn = fwf.silu.bin_from_sha(sha, bin_dir)
% By Filip Sz
%
% sha is a 1x64 character array produced by fwf.silu.sha_from_bin at the time
% of creating the bin file.
% bin_dir is the directory where a search is performed for .bin files with
% the correct sha

if nargin < 2
    bin_dir = fwf.silu.bin_default_directory;
end
...
for i = 1:numel(fnl)
    ...
    curr_sha = fwf.silu.sha_from_bin(curr_fn);
