function [gwf_out, dt_out] = gwf_resample_v1p23(gwf_in, dur_us)
% function [gwf_out, dt_out] = fwf.silu.gwf_resample_v1p23(gwf_in, dur_us)
% Output is single precision because the shape load requires float and 
% this is assumed to be the last operation performed.
% Near carbon copy of c++ code implemented in the v1.23 version of the
...
dt_in	  = double(dur_us) / ( double(n_samp_in)  - 1 );
dt_out    = double(dur_us) / ( double(n_samp_out) - 1 );

for i = int32(0:(n_samp_out-2))
    time			= double(i) * dt_out;
